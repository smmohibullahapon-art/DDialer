...
export default function BusinessVoipPage() {
  const [openFaq, setOpenFaq] = useState<number | null>(0);

  const faqs = [
    {
      q: "What is Business VoIP and how does it work?",
      a: "Business VoIP (Voice over Internet Protocol) converts your voice into digital packets, transmitting them securely over high-speed internet connections rather than traditional copper phone lines. DDialer's enterprise VoIP platform routes these calls through carrier-grade Kamailio and FreeSWITCH SBC cores."
    },
    {
      q: "Can I keep my existing business phone numbers?",
      a: "Yes! DDialer supports seamless number porting across local, national, and toll-free numbers in over 60+ countries with zero downtime to your existing operations."
    },
    {
      q: "Is Business VoIP secure for remote teams?",
      a: "Absolutely. DDialer implements TLS (Transport Layer Security) for signaling encryption and SRTP (Secure Real-time Transport Protocol) with 256-bit encryption for all voice media streams."
    },
    {
      q: "How does Business VoIP integrate with our CRM?",
      a: "DDialer provides robust webhooks and REST APIs that instantly synchronize call logs, customer recordings, and contact dispositions with leading CRM platforms."
    }
  ];
    ...
    <div className="min-h-screen bg-[#070913] text-[#8a99ad] selection:bg-[#00d2ff]/30 selection:text-white font-sans overflow-x-hidden relative">
      ...
      <nav className="fixed top-0 inset-x-0 z-50 bg-[#070913]/85 backdrop-blur-2xl border-b border-white/10 px-6 lg:px-16 h-20 flex items-center justify-between">
        ...
        <div className="hidden lg:flex items-center gap-8 text-xs font-mono">
          ...
          <Link href="/business-voip" className="text-white font-bold transition">Business VoIP</Link>
      ...
      <section className="relative pt-44 pb-28 px-6 lg:px-16 max-w-7xl mx-auto flex flex-col items-center text-center space-y-8">
        <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-white/5 border border-white/10 text-xs font-mono text-[#00ff88] shadow-inner backdrop-blur-md">
          ...
          <span>Next-Gen Business VoIP Phone System &amp; Cloud PBX</span>
        </div>

        <h1 className="text-4xl sm:text-6xl lg:text-7xl font-black text-white tracking-tight max-w-5xl leading-tight">
          Enterprise <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#00d2ff] to-[#00ff88]">Business VoIP</span> Phone Systems
        </h1>

        <p className="text-sm sm:text-base text-[#8a99ad] max-w-2xl leading-relaxed">
          Upgrade your organization with HD cloud-based voice communication. Eliminate legacy hardware, connect remote teams globally, and manage multi-region extensions under one sovereign platform by Dial Dynamic Ltd.
        </p>
      ...
      {/* 3. What is Business VoIP Section */}
      <section className="py-24 border-t border-white/10 bg-white/[0.01]">
        <div className="max-w-7xl mx-auto px-6 lg:px-16 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <span className="text-xs font-mono px-3 py-1 rounded bg-[#00d2ff]/10 text-[#00d2ff] border border-[#00d2ff]/30">Advanced Architecture</span>
            <h2 className="text-3xl font-bold text-white">What is Business VoIP &amp; Why Enterprises Switch</h2>
            <p className="text-xs sm:text-sm text-[#8a99ad] leading-relaxed">
              Traditional analog phone lines are costly, rigid, and restricted to physical office spaces. DDialer's Business VoIP phone system empowers modern enterprises with mobility, high-definition G.711/Opus audio quality, automated call routing, and real-time analytics.
            </p>
            <div className="space-y-3 font-mono text-xs">
              <div className="flex items-center gap-3 text-white">
                <Check className="w-4 h-4 text-[#00ff88]" />
                <span>Reduce monthly telecom expenses by up to 60%</span>
              </div>
              <div className="flex items-center gap-3 text-white">
                <Check className="w-4 h-4 text-[#00ff88]" />
                <span>Deploy remote agents and virtual extensions instantly</span>
              </div>
              <div className="flex items-center gap-3 text-white">
                <Check className="w-4 h-4 text-[#00ff88]" />
                <span>Enterprise-grade 99.999% carrier uptime SLA</span>
              </div>
            </div>
          </div>
          ...
          <div className="p-8 rounded-3xl bg-[#0b0f19] border border-white/15 space-y-6 shadow-2xl font-mono text-xs">
            <div className="flex items-center justify-between border-b border-white/10 pb-4">
              <span className="text-white font-bold">VoIP Performance Telemetry</span>
              <span className="text-[#00ff88] flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#00ff88] animate-pulse"></span> Optimal</span>
            </div>
            <div className="space-y-4">
              <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
                <span className="text-[10px] text-[#8a99ad]">Mean Opinion Score (MOS)</span>
                <div className="text-white font-bold text-base">4.88 / 5.00 (Crystal Clear)</div>
              </div>
              <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
                <span className="text-[10px] text-[#8a99ad]">Average Jitter &amp; Latency</span>
                <div className="text-[#00d2ff] font-bold text-base">1.4ms (Global Backbone)</div>
              </div>
              <div className="p-4 rounded-2xl bg-white/5 border border-white/10 space-y-1">
                <span className="text-[10px] text-[#8a99ad]">Signaling &amp; Media Security</span>
                <div className="text-[#00ff88] font-bold text-base">TLS / SRTP Encrypted</div>
              </div>
            </div>
          </div>
        </div>
      ...
      {/* 4. Core Business VoIP Features Grid */}
      <section className="py-28 px-6 lg:px-16 max-w-7xl mx-auto space-y-16">
        <div className="text-center space-y-3 max-w-2xl mx-auto">
          ...
          <h2 className="text-3xl sm:text-4xl font-bold text-white tracking-tight">Key Business VoIP Features</h2>
          <p className="text-xs sm:text-sm text-[#8a99ad] font-mono">Engineered to streamline corporate telephony and maximize agent productivity.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[
            { icon: Server, title: 'Cloud PBX & Extensions', desc: 'Manage department extensions, ring groups, and custom call forwarding without managing physical server racks.' },
            { icon: Globe, title: 'Virtual DIDs & Numbers', desc: 'Acquire local and international business numbers in USA, UK, Canada, and Europe instantly.' },
            { icon: Headphones, title: 'Call Center Integration', desc: 'Built-in queues, live agent monitoring, predictive dialing, and detailed call recordings.' },
            { icon: Lock, title: 'Encrypted SIP Trunking', desc: 'Connect your existing IP-PBX seamlessly to DDialer’s redundant upstream carrier network.' },
            { icon: Bot, title: 'AI Voice Assistants', desc: 'Deploy automated conversational bots to handle routine customer inquiries 24/7.' },
            { icon: Activity, title: 'Real-Time Analytics', desc: 'Monitor peak call hours, agent response times, and financial margins in real time.' }
          ].map((item, idx) => {
      ...
      <section className="py-24 border-t border-white/10 bg-white/[0.01]">
        <div className="max-w-4xl mx-auto px-6 lg:px-16 space-y-12">
          <div className="text-center space-y-3">
            ...
            <h2 className="text-3xl font-bold text-white">Business VoIP Frequently Asked Questions</h2>
            <p className="text-xs text-[#8a99ad] font-mono">Everything you need to know about transitioning to DDialer cloud telephony.</p>
          </div>
